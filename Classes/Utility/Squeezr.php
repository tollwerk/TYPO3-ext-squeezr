		...
		if (!self::$_headTag && !empty($tsfe->tmpl->setup['config.']['squeezr.'])) {
			...
			if (!empty($config['enable']) && intval($config['enable']) && !empty($config['images.']) && is_array($config['images.']) && !empty($config['css.']) && is_array($config['css.'])) {
				...
				if (preg_match("%^(\<head[^\>]*\>)(.*)$%i", $headTag, $headTagParts)) {
					...
					$tsfe->pSetup['headTag']			.= '<script type="text/javascript" id="squeezr" data-em-precision="'.min(10, max(0.001, floatval(empty($config['em2px']) ? 0.5 : $config['em2px']))).'"';
					...
					if (!empty($config['images.']['disable']) && intval($config['images.']['disable'])) {
						...
						$breakpoints					= (empty($config['images.']['breakpoints']) || !strlen(trim($config['images.']['breakpoints']))) ? array() : \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', trim($config['images.']['breakpoints']), true);
					...
					$tsfe->pSetup['headTag']			.= '>'.file_get_contents(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('squeezr', 'Resources'.DIRECTORY_SEPARATOR.'Private'.DIRECTORY_SEPARATOR.'Squeezr'.DIRECTORY_SEPARATOR.'squeezr'.DIRECTORY_SEPARATOR.'squeezr.min.js')).'</script>';
	...
	public function manipulateCacheActions(&$cacheActions, &$optionValues) {
		if ($GLOBALS['BE_USER']->isAdmin()) {
			...
			$cacheActions[]		= array(
				...
				'icon'			=> '<img '.\TYPO3\CMS\Backend\Utility\IconUtility::skinImg($GLOBALS['BACK_PATH'], \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extRelPath('squeezr').'ext_icon.gif', 'width="16" height="16"').' title="'.$title.'" alt="'.$title.'" />'
			);
